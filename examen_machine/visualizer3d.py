    ...
    def run(self, updater=None, dt=0.001):
        ...
        if rank == 0:
            ...
            while self.running:
                ...
                if comm.iprobe(source=1, tag=0):
                    new_points = comm.recv(source=1, tag=0)
                    self.update_points(new_points)
                    comm.send("ok", dest=1, tag=2) 

               
                t3 = sdl2.SDL_GetTicks()
                self._render()
                t2 = sdl2.SDL_GetTicks()
                print(f"Render time: {t2 - t3} ms", end='\r')
        ...
        #calcul
            ...
            while True:
                t1 = MPI.Wtime()
                new_points = updater(dt)
                t2 = MPI.Wtime()
                print(f"Update time: {(t2-t1)*1000:.1f} ms", end='\r')

                comm.send(new_points, dest=0, tag=0)
